...
const char * const BITCOIN_CONF_FILENAME = "asusucoin.conf";
...
#else
    const char* pszModule = "asusucoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Asusucoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Asusucoin";
#else
    // Unix
    return pathRet / ".asusucoin";
#endif
#endif
}
