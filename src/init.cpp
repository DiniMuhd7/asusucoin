 ...
 */
static const char* BITCOIN_PID_FILENAME = "asusucoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("asusucoin-shutoff");
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("asusucoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if asusucoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if asusucoin is started while in a temporary directory.\n",
